...
fn main_() -> Result<()> {
    ...
    let extra_sources = matches.values_of("source").map(Values::collect).unwrap_or(vec![]);

    let params: Vec<&str> = matches.values_of("query").map(Values::collect).context("Expected at least one query")?;

    println!("Base: {:?}, Extra: {:?}, Queries: {:?}", base_sources, extra_sources, params);

    let queries: Vec<Query> = params.iter().map(|&p| {
        match IpAddr::from_str(p) {
            Ok(ip) => Query::IP(ip),
            _ => Query::Domain(p.to_string())
        }
    }).collect();

    let result = queries.iter().flat_map(|query| {
        let sources = base_sources.iter().chain(extra_sources.iter());
        sources.map(move |&source| {
            lookup(source, query)
        })
    }).fold::<Result<i32>, _>(Ok(0), |r, i| {
        if i? {
            r.map(|n| { n+1 })
        } else {
            r
...
fn lookup(source: &str, query: &Query) -> Result<bool> {
    ...
    let proxy = conn.with_proxy("org.freedesktop.resolve1", "/org/freedesktop/resolve1", Duration::from_secs(30));
    ...
    let result: (Vec<(i32, i32, Vec<u8>)>, String, u64) = proxy.resolve_hostname(0, &hostname, libc::AF_INET, 0)?;
...
fn format_ip(ip: &IpAddr) -> String {
    match ip {
        IpAddr::V4(v4) => format!("{}.{}.{}.{}.", v4.octets()[3], v4.octets()[2], v4.octets()[1], v4.octets()[0]),
